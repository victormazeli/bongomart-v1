...
@if (is_array($supportedLanguages) && count($supportedLanguages) > 1)
	...
	<li class="dropdown lang-menu nav-item">
		<button type="button" class="btn btn-default dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false" id="langDropdown">
			<span class="lang-title">{{ strtoupper(config('app.locale')) }}</span>
		</button>
		<ul id="langDropdownItems"
			class="dropdown-menu {{ (config('lang.direction')=='rtl') ? 'dropdown-menu-start' : 'dropdown-menu-end' }} user-menu shadow-sm mt-2"
			role="menu"
